...
export function Claude(props) {
    ...
    const log = useMemo(() => createLogger('Claude', { model: props.model ?? 'sonnet' }), [props.model]);
    // TODO abstract all the following block of lines into named hooks
    const agentIdRef = useRef(null);
    const tailLogRef = useRef([]);
    const [, forceUpdate] = useReducer((x) => x + 1, 0);
    const { data: agentRows } = useQuery(reactiveDb, "SELECT status, result, result_structured, error, tokens_input, tokens_output, duration_ms FROM agents WHERE id = ?", [agentIdRef.current ?? '']);
    ...
    const result = agentRow?.result ? {
        ...
        structured: agentRow.result_structured ? (() => { try {
    ...
    const messageParserRef = useRef(new MessageParser(maxEntries * 2));
    const isMounted = useMountedState();
    const lastTailLogUpdateRef = useRef(0);
    const pendingTailLogUpdateRef = useRef(null);
        ...
        (async () => {
            ...
            try {
                ...
                if (props.model !== undefined)
                    baseOptions.model = props.model;
                if (props.permissionMode !== undefined)
                    baseOptions.permissionMode = props.permissionMode;
                if (props.maxTurns !== undefined)
                    baseOptions.maxTurns = props.maxTurns;
                if (props.systemPrompt !== undefined)
                    baseOptions.systemPrompt = props.systemPrompt;
                if (props.outputFormat !== undefined)
                    baseOptions.outputFormat = props.outputFormat;
                if (mcpConfigPath !== undefined)
                    baseOptions.mcpConfig = mcpConfigPath;
                if (props.allowedTools !== undefined)
                    baseOptions.allowedTools = props.allowedTools;
                if (props.disallowedTools !== undefined)
                    baseOptions.disallowedTools = props.disallowedTools;
                if (cwd !== undefined)
                    baseOptions.cwd = cwd;
                if (props.timeout !== undefined)
                    baseOptions.timeout = props.timeout;
                if (props.stopConditions !== undefined)
                    baseOptions.stopConditions = props.stopConditions;
                if (props.continueConversation !== undefined)
                    baseOptions.continue = props.continueConversation;
                if (props.resumeSession !== undefined)
                    baseOptions.resume = props.resumeSession;
                if (props.onToolCall !== undefined)
                    baseOptions.onToolCall = props.onToolCall;
                if (props.schema !== undefined)
                    baseOptions.schema = props.schema;
                if (props.schemaRetries !== undefined)
                    baseOptions.schemaRetries = props.schemaRetries;
                if (props.useSubscription !== undefined)
                    baseOptions.useSubscription = props.useSubscription;
                if (typedStreamingEnabled && props.outputFormat === undefined)
                    baseOptions.outputFormat = 'stream-json';
                ...
                if (props.reportingEnabled !== false) {
                    ...
                    currentAgentId = await db.agents.start(executionOptions.prompt, executionOptions.model ?? 'sonnet', executionOptions.systemPrompt, logPath);
                ...
                forceUpdate();
                if (props.reportingEnabled !== false && currentAgentId) {
                    await db.agents.complete(currentAgentId, agentResult.output, agentResult.structured, agentResult.tokensUsed);
                }
    ...
    return (_jsxs("claude", { status: status, agentId: agentIdRef.current, executionId: executionId, model: props.model ?? 'sonnet', ...(error?.message ? { error: error.message } : {}), ...(result?.tokensUsed?.input !== undefined ? { tokensInput: result.tokensUsed.input } : {}), ...(result?.tokensUsed?.output !== undefined ? { tokensOutput: result.tokensUsed.output } : {}), ...(result?.turnsUsed !== undefined ? { turnsUsed: result.turnsUsed } : {}), ...(result?.durationMs !== undefined ? { durationMs: result.durationMs } : {}), children: [displayEntries.length > 0 && (_jsx("messages", { count: displayEntries.length, children: displayEntries.map(entry => entry.type === 'message' ? (_jsx("message", { index: entry.index, children: truncateToLastLines(entry.content, maxLines) }, entry.index)) : (_jsx("tool-call", { name: entry.toolName, index: entry.index, children: truncateToLastLines(entry.content, maxLines) }, entry.index))) })), props.children] }));
}
