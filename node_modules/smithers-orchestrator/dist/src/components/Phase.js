...
function getPhaseStatus(isSkipped, isActive, isCompleted, hasError) {
    if (hasError)
        return 'error';
    if (isSkipped)
        return 'skipped';
...
export function Phase(props) {
    ...
    const skipIfErrorRef = useRef(null);
    // Create logger with phase context
    const log = useMemo(() => createLogger('Phase', { name: props.name }), [props.name]);
    ...
    useEffect(() => {
        if (isSkipped || hasError || !executionEnabled)
            return;
    ...
    return (_jsx("phase", { name: props.name, status: status, ...(skipIfErrorRef.current ? { error: skipIfErrorRef.current.message } : {}), children: shouldRenderChildren && (_jsx(ExecutionBoundary, { enabled: isActive, children: _jsx(StepRegistryProvider, { phaseId: props.name, onAllStepsComplete: handleAllStepsComplete, children: props.children }) })) }));
}
