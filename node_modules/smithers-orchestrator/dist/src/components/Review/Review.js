 ...
 */
async function fetchTargetContent(target) {
    switch (target.type) {
        case 'commit': {
 ...
 */
function buildReviewPrompt(content, criteria) {
 ...
 */
async function executeReview(prompt, model) {
 ...
 */
async function postToGitHubPR(prNumber, review) {
...
export function Review(props) {
            ...
            try {
                ...
                if (isMounted()) {
                    ...
                    if (props.blocking && !reviewResult.approved) {
                        ...
                        smithers.requestStop(`Review failed: ${criticalCount} critical, ${majorCount} major issues found. ${reviewResult.summary}`);
                    }
                }
            }
            catch (err) {
    ...
    return (_jsx("review", { status: statusRef.current, approved: resultRef.current?.approved, summary: resultRef.current?.summary, "issue-count": resultRef.current?.issues.length, error: errorRef.current?.message, "target-type": props.target.type, "target-ref": props.target.ref, blocking: props.blocking }));
}
