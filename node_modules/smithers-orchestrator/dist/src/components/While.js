...
const WhileIterationContext = createContext(null);
export function useWhileIteration() {
    return useContext(WhileIterationContext);
}
export function While(props) {
    ...
    const { data: iteration } = useQueryValue(db.db, "SELECT json_extract(value, '$') as v FROM state WHERE key = ?", [iterationKey]);
    const iterationValue = iteration ?? 0;
    const { data: status } = useQueryValue(db.db, "SELECT json_extract(value, '$') as v FROM state WHERE key = ?", [statusKey]);
    ...
    return (_jsx("while", { id: whileId, iteration: iterationValue, status: statusValue, maxIterations: maxIterations, children: isRunning && (_jsx(WhileIterationContext.Provider, { value: contextValue, children: props.children })) }));
}
