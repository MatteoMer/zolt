...
import { generateStructuredOutputPrompt, generateRetryPrompt, parseStructuredOutput, } from '../../../utils/structured-output.js';
 ...
 */
function shouldFallbackToApiKey(stdout, stderr, exitCode) {
    if (exitCode === 0)
        return false;
    const combined = `${stdout}\n${stderr}`.toLowerCase();
    return (combined.includes('subscription') ||
 ...
 */
export async function executeClaudeCLIOnce(options, startTime, useApiKey = false) {
    ...
    try {
        ...
        const readStream = async () => {
            while (true) {
                ...
                const { shouldStop, reason: _reason } = checkStopConditions(options.stopConditions, partialResult);
                if (shouldStop) {
                    killed = true;
                    proc?.kill();
...
export async function executeClaudeCLI(options) {
    ...
    while (schemaRetryCount < maxSchemaRetries) {
        ...
        schemaRetryCount++;
        options.onProgress?.(`Schema validation failed (attempt ${schemaRetryCount}/${maxSchemaRetries}): ${parseResult.error}`);
 ...
 */
export async function executeClaudeShell(prompt, options = {}) {
    const startTime = Date.now();
