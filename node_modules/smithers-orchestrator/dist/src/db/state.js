// State management module for Smithers DB
import { uuid, now, parseJson } from './utils.js';
export function createStateModule(ctx) {
    const { rdb, getCurrentExecutionId } = ctx;
    const state = {
        get: (key) => {
            if (rdb.isClosed)
                return null;
            const row = rdb.queryOne('SELECT value FROM state WHERE key = ?', [key]);
            return row ? parseJson(row.value, null) : null;
        },
        set: (key, value, trigger) => {
            if (rdb.isClosed)
                return;
            const oldValue = state.get(key);
            const jsonValue = JSON.stringify(value);
            rdb.run('INSERT INTO state (key, value, updated_at) VALUES (?, ?, ?) ON CONFLICT(key) DO UPDATE SET value = ?, updated_at = ?', [key, jsonValue, now(), jsonValue, now()]);
            rdb.invalidate(['state']);
            // Log transition
            const currentExecutionId = getCurrentExecutionId();
            if (currentExecutionId) {
                rdb.run('INSERT INTO transitions (id, execution_id, key, old_value, new_value, trigger, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [uuid(), currentExecutionId, key, JSON.stringify(oldValue), jsonValue, trigger ?? null, now()]);
            }
        },
        setMany: (updates, trigger) => {
            if (rdb.isClosed)
                return;
            for (const [key, value] of Object.entries(updates)) {
                state.set(key, value, trigger);
            }
        },
        getAll: () => {
            if (rdb.isClosed)
                return {};
            const rows = rdb.query('SELECT key, value FROM state');
            const result = {};
            for (const row of rows) {
                result[row.key] = parseJson(row.value, null);
            }
            return result;
        },
        reset: () => {
            if (rdb.isClosed)
                return;
            rdb.run('DELETE FROM state');
            rdb.run("INSERT INTO state (key, value) VALUES ('phase', '\"initial\"'), ('ralphCount', '0'), ('data', 'null')");
        },
        history: (key, limit = 100) => {
            if (rdb.isClosed)
                return [];
            if (key) {
                return rdb.query('SELECT * FROM transitions WHERE key = ? ORDER BY created_at DESC LIMIT ?', [key, limit]);
            }
            return rdb.query('SELECT * FROM transitions ORDER BY created_at DESC LIMIT ?', [limit]);
        },
        has: (key) => {
            if (rdb.isClosed)
                return false;
            const row = rdb.queryOne('SELECT COUNT(*) as count FROM state WHERE key = ?', [key]);
            return (row?.count ?? 0) > 0;
        },
        delete: (key, trigger) => {
            if (rdb.isClosed)
                return;
            const oldValue = state.get(key);
            if (oldValue === null)
                return; // Key doesn't exist
            rdb.run('DELETE FROM state WHERE key = ?', [key]);
            rdb.invalidate(['state']);
            // Log transition
            const currentExecutionId = getCurrentExecutionId();
            if (currentExecutionId) {
                rdb.run('INSERT INTO transitions (id, execution_id, key, old_value, new_value, trigger, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [uuid(), currentExecutionId, key, JSON.stringify(oldValue), 'null', trigger ?? 'delete', now()]);
            }
        },
    };
    return state;
}
//# sourceMappingURL=state.js.map