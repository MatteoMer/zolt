...
export class LogWriter {
    constructor(logDir = '.smithers/logs', executionId) {
        this.counter = 0;
    ...
    writeLog(type, content, metadata) {
        this.counter++;
     ...
     */
    appendStreamPart(filename, part) {
        const filepath = path.join(this.logDir, filename);
        let stream = this.streams.get(filename);
        if (!stream) {
            stream = fs.createWriteStream(filepath, { flags: 'a', encoding: 'utf-8' });
     ...
     */
    writeStreamSummary(filename, parts) {
        const filepath = path.join(this.logDir, filename);
     ...
     */
    writeStreamSummaryFromCounts(filename, summary) {
        const filepath = path.join(this.logDir, filename);
        const summaryPath = filepath.replace(/\.log$|\.ndjson$/, '') + '.summary.json';
        fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));
        return summaryPath;
