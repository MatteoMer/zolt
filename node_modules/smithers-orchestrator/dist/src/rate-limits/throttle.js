export class ThrottleController {
    ...
    async acquire(provider, model) {
        ...
        if (capacity.overall <= 0) {
            ...
            const resetTime = Math.min(status.requests.resetsAt.getTime(), status.inputTokens.resetsAt.getTime(), status.outputTokens.resetsAt.getTime());
