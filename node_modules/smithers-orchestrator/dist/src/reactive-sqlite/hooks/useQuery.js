 ...
 * ```
 */
export function useQuery(sqlOrDb, sqlOrParams, paramsOrOptions, optionsOrDb) {
    ...
    if (typeof sqlOrDb === 'string') {
        ...
        options = (Array.isArray(sqlOrParams) ? paramsOrOptions : sqlOrParams) ?? {};
    ...
    const { cacheRef, invalidateCache } = useQueryCache();
    ...
    const queryKey = useMemo(() => JSON.stringify({ sql, params, skip, dbId }), [sql, JSON.stringify(params), skip, dbId]);
    ...
    const snapshot = useSyncExternalStore(subscribe, getSnapshot, getSnapshot // Server snapshot (same as client for SQLite)
    );
