 ...
 */
export function extractReadTables(sql) {
 ...
 */
export function extractWriteTables(sql) {
    const tables = new Set();
    const normalized = sql
        .replace(/--.*$/gm, '')
        .replace(/\/\*[\s\S]*?\*\//g, '')
 ...
 */
export function isWriteOperation(sql) {
 ...
 */
export function extractAllTables(sql) {
    return {
 ...
 */
export function extractRowFilter(sql, params = []) {
    const normalized = sql
        .replace(/--.*$/gm, '') // Remove single-line comments
        .replace(/\/\*[\s\S]*?\*\//g, '') // Remove multi-line comments
        .replace(/\s+/g, ' ') // Normalize whitespace
    ...
    const equalityMatch = firstCondition.match(/^(?:"([^"]+)"|([a-z_][a-z0-9_]*))\s*=\s*(.+)$/i);
