 ...
 * License: MIT
 */
import { createElement, createContext, useCallback, useContext, useEffect, useRef, } from "react";
const ExecutionGateContext = createContext(true);
export function ExecutionGateProvider(props) {
 ...
 * });
 */
export function useEffectOnValueChange(value, effect, deps = []) {
 ...
 * }, [someDep]);
 */
export function useExecutionMount(executionEnabled, fn, deps = []) {
    const hasStartedRef = useRef(false);
    useEffect(() => {
        if (!executionEnabled || hasStartedRef.current)
            return;
