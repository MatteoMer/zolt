 ...
 */
function addWarningsForUnknownParents(node) {
    ...
    while (parent) {
        ...
        if (parent.type !== 'ROOT') {
            if (isKnown) {
                node.warnings.push(`<${node.type}> rendered inside unknown element <${parent.type}>`);
            }
            break;
        }
 ...
 */
function containsFunctions(value, seen = new WeakSet()) {
    ...
    return Object.values(value).some(v => containsFunctions(v, seen));
}
/**
