...
export function createClaudeAssistant(db) {
    const apiKey = process.env['ANTHROPIC_API_KEY'];
    const client = apiKey ? new Anthropic({ apiKey }) : null;
    const getContextSummary = () => {
        try {
            const execution = db.execution.current();
            const phases = db.query('SELECT * FROM phases ORDER BY created_at DESC LIMIT 5');
            const agents = db.query('SELECT * FROM agents ORDER BY created_at DESC LIMIT 5');
            const toolCalls = db.query('SELECT * FROM tool_calls ORDER BY created_at DESC LIMIT 10');
            const recentFrames = db.query('SELECT * FROM render_frames ORDER BY created_at DESC LIMIT 3');
            return `
Current Context:
...
//# sourceMappingURL=claude-assistant.js.map