...
function computeScores(content) {
    const scores = {
        review: 0,
        issue: 0,
        todo: 0,
    ...
    if (hasCommitHash)
        scores.review += 0.4;
    if (fileRefCount > 0)
        scores.review += Math.min(fileRefCount * 0.15, 0.3);
    if (negativeCount > 0)
        scores.review += Math.min(negativeCount * 0.1, 0.2);
    if (reviewWordCount > 0)
        scores.review += Math.min(reviewWordCount * 0.05, 0.1);
    // Issue signals
    ...
    if (!hasCommitHash && futureVerbCount > 0)
        scores.issue += 0.3;
    if (modalVerbCount > 0)
        scores.issue += Math.min(modalVerbCount * 0.1, 0.2);
    if (featureWordCount > 0)
        scores.issue += Math.min(featureWordCount * 0.15, 0.3);
    if (designWordCount > 0)
        scores.issue += Math.min(designWordCount * 0.1, 0.2);
    // TODO signals
    ...
    if (hasCheckbox)
        scores.todo += 0.5;
    if (urgentCount > 0)
        scores.todo += Math.min(urgentCount * 0.15, 0.3);
    if (hasImperative)
        scores.todo += 0.2;
    if (actionWordCount > 0)
        scores.todo += Math.min(actionWordCount * 0.1, 0.2);
    return scores;
}
export function classifyContent(ctx) {
    ...
    // Build reasoning
    if (maxType === 'review') {
        if (PATTERNS.review.commitHash.test(content))
            reasoning.push('Commit hash detected in content');
        if (PATTERNS.review.fileRefs.test(content))
            reasoning.push('File:line references found');
        if (PATTERNS.review.negativeWords.test(content))
            reasoning.push('Negative/bug language present');
    }
    else if (maxType === 'issue') {
        if (PATTERNS.issue.futureVerbs.test(content))
            reasoning.push('Future-tense verbs detected');
        if (PATTERNS.issue.featureWords.test(content))
            reasoning.push('Feature/enhancement language');
        if (!PATTERNS.review.commitHash.test(content))
            reasoning.push('No commit references');
    }
    else if (maxType === 'todo') {
        if (PATTERNS.todo.checkbox.test(content))
            reasoning.push('Checkbox pattern detected');
        if (PATTERNS.todo.urgentWords.test(content))
            reasoning.push('Urgent language present');
        if (PATTERNS.todo.imperative.test(content))
            reasoning.push('Imperative mood detected');
    }
...
// ============================================================================
export function extractCommitHash(text) {
...
// Template Generation
// ============================================================================
export function generateReviewTemplate(ctx) {
...
export function generateIssueTemplate(ctx) {
...
export function generateTodoItem(ctx) {
    const priority = ctx.priority ?? inferPriority(ctx.content);
    const content = ctx.content.trim();
...
export function generatePromptMd(ctx) {
...
export async function generateFilePath(type, ctx, cwd) {
    switch (type) {
        case 'review': {
...
// File Writing
// ============================================================================
export async function writeCapture(generated) {
