...
pub fn parseHexIntoBuffer(text: []const u8, buffer: []u8) ![]const u8 {
    const trimmed = std.mem.trim(u8, text, " \t\r");
    if (trimmed.len % 2 != 0) return error.InvalidHexLength;

    const byte_len = trimmed.len / 2;
    if (byte_len > buffer.len) return error.BufferTooSmall;

    _ = try std.fmt.hexToBytes(buffer[0..byte_len], trimmed);
