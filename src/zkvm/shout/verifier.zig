//! Shout Verifier for Instruction Lookup Arguments
//!
//! This module implements the Shout verifier for verifying instruction lookups
...
/// Shout verifier state
pub fn ShoutVerifier(comptime F: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn init(allocator: Allocator, params: ShoutParams(F), initial_claim: F) !Self {
...
/// Verify a complete Shout proof
pub fn verifyShoutProof(
    comptime F: type,
    allocator: Allocator,
    proof: *const ShoutProof(F),
    params: ShoutParams(F),
    initial_claim: F,
) !bool {
    var verifier = try ShoutVerifier(F).init(allocator, params, initial_claim);
...
/// Batched verification for multiple Shout proofs
pub fn batchVerifyShoutProofs(
    comptime F: type,
    allocator: Allocator,
    proofs: []const ShoutProof(F),
    params: []const ShoutParams(F),
    ...
    for (proofs, params, initial_claims) |proof, param, claim| {
        const valid = try verifyShoutProof(F, allocator, &proof, param, claim);
        if (!valid) {
            all_valid = false;
            break;
        }
...
test "shout verifier basic" {
    ...
    var verifier = try ShoutVerifier(F).init(allocator, params, F.fromU64(10));
    defer verifier.deinit();
