...
const stage3_instances = @import("stage3_instances.zig");
const stage3_instruction_input = @import("stage3_instruction_input.zig");
    ...
    return struct {
        ...
        pub fn generateStage3Proof(
            ...
            if (comptime debug_verbose) {
                dbg("[STAGE3] generateStage3Proof: raw_inputs.len = {}\n", .{raw_inputs.len});
            ...
            if (comptime debug_verbose) {
                ...
                for (0..trace_len) |jj| {
                    if (jj < raw_inputs.len) {
                        const rs1_val = raw_inputs[jj].toFieldValue(F, .Rs1Value);
                ...
                // Build from the raw R1CS inputs directly.
                var rd_mle_sum = F.zero();
                var rs1_mle_sum = F.zero();
                var rs2_mle_sum = F.zero();
                for (0..trace_len) |jj| {
                    if (jj < raw_inputs.len) {
                        const rd_val = raw_inputs[jj].toFieldValue(F, .RdWriteValue);
                        const rs1_val = raw_inputs[jj].toFieldValue(F, .Rs1Value);
                        const rs2_val = raw_inputs[jj].toFieldValue(F, .Rs2Value);
                ...
                for (0..trace_len) |jj| {
                    if (jj < raw_inputs.len) {
                        manual_rd[jj] = raw_inputs[jj].toFieldValue(F, .RdWriteValue);
                    } else {
                        manual_rd[jj] = F.zero();
                    }
                }
...
// Sub-prover types (extracted to separate files for navigability)
fn ShiftPrefixSuffixProver(comptime F: type) type {
    return stage3_instances.ShiftPrefixSuffixProver(F);
}

fn InstructionInputProver(comptime F: type) type {
    return stage3_instruction_input.InstructionInputProver(F);
}

fn RegistersPrefixSuffixProver(comptime F: type) type {
    return stage3_instances.RegistersPrefixSuffixProver(F);
}
