    ...
    return struct {
        ...
        pub fn init(
            ...
            if (comptime debug_verbose) {
                ...
                // Verify partition of unity: Σ eq(r_product, j) = 1
                {
                    const po2 = @import("../../poly/mod.zig");
                    const eq_prod_check = try po2.EqPolynomial(F).evalsSliceWithScaling(F, allocator, r_product, null);
                    defer allocator.free(eq_prod_check);
                    var eq_sum = F.zero();
                    for (eq_prod_check) |e| eq_sum = eq_sum.add(e);
                    std.debug.print("PARTITION OF UNITY: Σ eq(r_product,j) = {any} (should be 1)\n", .{eq_sum.toBytesBE()});
                    std.debug.print("PARTITION OF UNITY: is_one? {}\n", .{eq_sum.eql(F.one())});

                    // Also check: Σ eq_plus_one(r_product, j) = 1 - eq(r_product, T-1)
                    const ep_check = try allocator.alloc(F, trace_len);
                    defer allocator.free(ep_check);
                    po2.EqPolynomial(F).buildEqPlusOneTableInPlace(r_product, ep_check);
                    var ep_sum = F.zero();
                    for (ep_check) |e| ep_sum = ep_sum.add(e);
                    const eq_T_minus_1 = eq_prod_check[trace_len - 1];
                    std.debug.print("Σ eq_plus_one(r_prod,j) = {any}\n", .{ep_sum.toBytesBE()[0..8]});
                    std.debug.print("1 - eq(r_prod,T-1) = {any}\n", .{F.one().sub(eq_T_minus_1).toBytesBE()[0..8]});
                    std.debug.print("match? {}\n", .{ep_sum.eql(F.one().sub(eq_T_minus_1))});
                }

                // Compare noop terms: compute direct NextIsNoop MLE at r_product
                {
                    const po2 = @import("../../poly/mod.zig");
                    const eq_prod2 = try po2.EqPolynomial(F).evalsSliceWithScaling(F, allocator, r_product, null);
                    defer allocator.free(eq_prod2);

                    // Compute NextIsNoop(r_product) directly from witness
                    var next_is_noop_direct = F.zero();
                    for (0..trace_len) |j| {
                        // NextIsNoop(j) = IsNoop(j+1)
                        const nin = if (j + 1 < trace_len)
                            cycle_witnesses[j + 1].values[R1CSInputIndex.FlagIsNoop.toIndex()]
                        else F.one();
                        next_is_noop_direct = next_is_noop_direct.add(eq_prod2[j].mul(nin));
                    }

                    // Prover's noop sum: Σ eq_plus_one(r_prod, j) * (1-IsNoop(j))
                    const ep2 = try allocator.alloc(F, trace_len);
                    defer allocator.free(ep2);
                    po2.EqPolynomial(F).buildEqPlusOneTableInPlace(r_product, ep2);
                    var prover_noop = F.zero();
                    for (0..trace_len) |j| {
                        const noop_j = cycle_witnesses[j].values[R1CSInputIndex.FlagIsNoop.toIndex()];
                        prover_noop = prover_noop.add(ep2[j].mul(F.one().sub(noop_j)));
                    }

                    const input_noop = F.one().sub(next_is_noop_direct);
                    std.debug.print("NOOP: prover={any} input={any} match={}\n", .{
                        prover_noop.toBytesBE()[0..8], input_noop.toBytesBE()[0..8], prover_noop.eql(input_noop),
                    });

                    // Also check: outer terms
                    const ep_outer2 = try allocator.alloc(F, trace_len);
                    defer allocator.free(ep_outer2);
                    po2.EqPolynomial(F).buildEqPlusOneTableInPlace(r_outer, ep_outer2);
                    const eq_outer2 = try po2.EqPolynomial(F).evalsSliceWithScaling(F, allocator, r_outer, null);
                    defer allocator.free(eq_outer2);

                    var prover_upc = F.zero();
                    var input_upc = F.zero();
                    for (0..trace_len) |j| {
                        const upc_j = cycle_witnesses[j].values[R1CSInputIndex.UnexpandedPC.toIndex()];
                        const nupc_j = cycle_witnesses[j].values[R1CSInputIndex.NextUnexpandedPC.toIndex()];
                        prover_upc = prover_upc.add(ep_outer2[j].mul(upc_j));
                        input_upc = input_upc.add(eq_outer2[j].mul(nupc_j));
                    }
                    var prover_pc = F.zero();
                    var input_pc = F.zero();
                    var prover_virt = F.zero();
                    var input_virt = F.zero();
                    var prover_first = F.zero();
                    var input_first = F.zero();
                    for (0..trace_len) |j| {
                        const w2 = cycle_witnesses[j].values;
                        prover_pc = prover_pc.add(ep_outer2[j].mul(w2[R1CSInputIndex.PC.toIndex()]));
                        input_pc = input_pc.add(eq_outer2[j].mul(w2[R1CSInputIndex.NextPC.toIndex()]));
                        prover_virt = prover_virt.add(ep_outer2[j].mul(w2[R1CSInputIndex.FlagVirtualInstruction.toIndex()]));
                        input_virt = input_virt.add(eq_outer2[j].mul(w2[R1CSInputIndex.NextIsVirtual.toIndex()]));
                        prover_first = prover_first.add(ep_outer2[j].mul(w2[R1CSInputIndex.FlagIsFirstInSequence.toIndex()]));
                        input_first = input_first.add(eq_outer2[j].mul(w2[R1CSInputIndex.NextIsFirstInSequence.toIndex()]));
                    }
                    std.debug.print("UPC:   prover={any} input={any} match={}\n", .{prover_upc.toBytesBE()[0..8], input_upc.toBytesBE()[0..8], prover_upc.eql(input_upc)});
                    std.debug.print("PC:    prover={any} input={any} match={}\n", .{prover_pc.toBytesBE()[0..8], input_pc.toBytesBE()[0..8], prover_pc.eql(input_pc)});
                    std.debug.print("VIRT:  prover={any} input={any} match={}\n", .{prover_virt.toBytesBE()[0..8], input_virt.toBytesBE()[0..8], prover_virt.eql(input_virt)});
                    std.debug.print("FIRST: prover={any} input={any} match={}\n", .{prover_first.toBytesBE()[0..8], input_first.toBytesBE()[0..8], prover_first.eql(input_first)});
                    // Find ALL mismatches
                    var first_mm: usize = 0;
                    for (0..trace_len - 1) |j| {
                        const nif_j = cycle_witnesses[j].values[R1CSInputIndex.NextIsFirstInSequence.toIndex()];
                        const fif_j1 = cycle_witnesses[j + 1].values[R1CSInputIndex.FlagIsFirstInSequence.toIndex()];
                        if (!nif_j.eql(fif_j1)) {
                            if (first_mm < 3) {
                                const w_j1 = cycle_witnesses[j + 1].values;
                                const dnupc = w_j1[R1CSInputIndex.FlagDoNotUpdateUnexpandedPC.toIndex()].toU64();
                                const is_comp = w_j1[R1CSInputIndex.FlagIsCompressed.toIndex()].toU64();
                                std.debug.print("FIRST MISMATCH: j={d} NextIsFirst[j]={x} IsFirst[j+1]={x} VI[j+1]={x} DNUPC={x} IsComp={x}\n", .{
                                    j, nif_j.toU64(), fif_j1.toU64(),
                                    w_j1[R1CSInputIndex.FlagVirtualInstruction.toIndex()].toU64(),
                                    dnupc, is_comp,
                                });
                            }
                            first_mm += 1;
                    ...
                    std.debug.print("Total FIRST mismatches: {d}\n", .{first_mm});
                }
