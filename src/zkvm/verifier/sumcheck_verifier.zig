...
///   2. Verify degree: compressed length must equal max_degree (d coefficients for degree d)
///   3. Recover linear term: c1 = claim - 2*c0 - c2 - c3 - ... - c_d
...
pub fn verifySumcheck(
    ...
    for (0..num_rounds) |round| {
        ...
        // Degree bound check: compressed form has d coefficients for a degree-d polynomial
        // (stores [c0, c2, c3, ..., c_d], omitting c1)
        if (compressed.len != max_degree) {
            return error.DegreeBoundExceeded;
        }
...
/// The verifier checks the degree bound, appends it to the transcript,
/// derives a challenge, and evaluates the polynomial at that challenge.
///
/// Returns the evaluated claim and the challenge, or error if degree exceeded.
pub fn verifyUniSkipRound(
    comptime F: type,
    uni_poly: []const F,
    degree_bound: usize,
    transcript: anytype,
) error{DegreeBoundExceeded}!struct { claim: F, challenge: F } {
    // Degree bound check: polynomial of degree d has d+1 coefficients
    if (uni_poly.len > degree_bound + 1) {
        return error.DegreeBoundExceeded;
    }
...
test "sumcheck verifier: single round degree-3" {
    ...
    const Interp = zolt_arith.poly.interpolation.Interpolation(F_);

    // Simulate a prover: create a degree-3 polynomial and compress it
    // p(x) = 5 + 3x + 2x^2 + x^3
    // p(0) = 5, p(1) = 5+3+2+1 = 11
    // claim = p(0) + p(1) = 16
    const claim = F_.fromU64(16);

    // Compressed form: [c0, c2, c3] = [5, 2, 1] (excludes c1=3)
    var coeffs = [_]F_{ F_.fromU64(5), F_.fromU64(2), F_.fromU64(1) };
    ...
    var prover_transcript = Blake2bTranscript(F_).init("test");
    ...
    var verifier_transcript = Blake2bTranscript(F_).init("test");
    const result = try verifySumcheck(
        F_,
        &proof,
        claim,
        1,
        3, // max_degree = 3 (degree-3 polynomial)
        &verifier_transcript,
        testing.allocator,
    );
...
test "sumcheck verifier: degree bound rejection" {
    const F_ = BN254Scalar;

    // Create a degree-3 polynomial proof
    const full_coeffs = [_]F_{ F_.fromU64(5), F_.fromU64(3), F_.fromU64(2), F_.fromU64(1) };
    var proof = jolt_types.SumcheckInstanceProof(F_).init(testing.allocator);
    defer proof.deinit();
    try proof.addRoundPoly(&full_coeffs);

    // Try to verify with max_degree=2 — should fail
    var transcript = Blake2bTranscript(F_).init("test");
    const result = verifySumcheck(
        F_,
        &proof,
        F_.fromU64(16),
        1,
        2, // max_degree = 2, but poly is degree 3
        &transcript,
        testing.allocator,
    );
    try testing.expectError(error.DegreeBoundExceeded, result);
}
...
test "sumcheck verifier: multi-round" {
    ...
    //   compressed = [10, 2]
    const claim1 = F_.fromU64(26);
    const full1 = [_]F_{ F_.fromU64(10), F_.fromU64(4), F_.fromU64(2) };

    // Create transcript to derive challenge for round 1
    var sim_transcript = Blake2bTranscript(F_).init("multi");
    sim_transcript.appendScalars("sumcheck_poly", &[_]F_{ F_.fromU64(10), F_.fromU64(2) });
    const r1 = sim_transcript.challengeScalar();

    // Round 2: p(x) = 3 + x + 5x^2
    const full2 = [_]F_{ F_.fromU64(3), F_.fromU64(1), F_.fromU64(5) };
    ...
    // Verify
    var verifier_transcript = Blake2bTranscript(F_).init("multi");
    const result = try verifySumcheck(F_, &proof, claim1, 2, 2, &verifier_transcript, testing.allocator);
    defer testing.allocator.free(result.challenges);
...
test "uni skip round verification" {
    ...
    const poly_ = [_]F_{ F_.fromU64(1), F_.fromU64(2), F_.fromU64(3) };

    var transcript = Blake2bTranscript(F_).init("skip");
    const result = try verifyUniSkipRound(F_, &poly_, 2, &transcript);

    // Manually compute: append, derive challenge, evaluate
    var expected_transcript = Blake2bTranscript(F_).init("skip");
    expected_transcript.appendScalars("uniskip_poly", &poly_);
    const expected_challenge = expected_transcript.challengeScalar();

    // Horner: 1 + 2*r + 3*r^2
    const expected_eval = F_.fromU64(1)
        .add(F_.fromU64(2).mul(expected_challenge))
        .add(F_.fromU64(3).mul(expected_challenge.mul(expected_challenge)));

    try testing.expect(result.challenge.eql(expected_challenge));
    try testing.expect(result.claim.eql(expected_eval));
}

test "uni skip degree bound rejection" {
    const F_ = BN254Scalar;

    // Polynomial of degree 3 (4 coefficients)
    const poly_ = [_]F_{ F_.fromU64(1), F_.fromU64(2), F_.fromU64(3), F_.fromU64(4) };

    var transcript = Blake2bTranscript(F_).init("skip");
    // degree_bound=2, but poly has degree 3 → should fail
    const result = verifyUniSkipRound(F_, &poly_, 2, &transcript);
    try testing.expectError(error.DegreeBoundExceeded, result);
}
